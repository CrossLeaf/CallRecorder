...
open class CallRecordReceiver(private var callRecord: CallRecord) : PhoneCallReceiver() {
    ...
    override fun onIncomingCallEnded(
        ...
        LogUtils.d("onIncomingCallEnded sleep")
        Thread.sleep(3000)
        LogUtils.d("onIncomingCallEnded wake up")
        stopRecord(context)
    }
    ...
    override fun onOutgoingCallEnded(
        ...
        LogUtils.d("onOutgoingCallEnded")
        Thread.sleep(3000)
        LogUtils.d("onOutgoingCallEnded wake up")
        stopRecord(context)
    }
    ...
    protected open fun onRecordingStarted(
        context: Context, callRecord: CallRecord, audioFile: File?
    ) {
    }

    protected open fun onRecordingFinished(
        context: Context, callRecord: CallRecord, audioFile: File?
    ) {
    }

    private fun startRecord(context: Context, seed: String, phoneNumber: String?) {
        LogUtils.i(TAG, "startRecord: $phoneNumber")
    ...
    private fun prepareAudioRecorder(
        ...
        try {
            ...
            LogUtils.d(TAG, "prepareAudioRecorder: $dirPath/$dirName")
            ...
            LogUtils.d("音檔絕對路徑：${audioFile!!.absolutePath}")
